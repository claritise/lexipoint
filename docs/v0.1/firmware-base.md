...
| `src/activities/reader/EpubReaderMenuActivity.{h,cpp}` | (P10) no Look Up row with LEXIRISE (`lookup-flow.md` §5g). (P9) the `LOOKUP_LANGUAGE` action and its row (after Look Up; P10: in its place), cycled in place like Night mode (`BookLanguageRow`), `setBookPath()`, `bookLanguageLabel()`. **Not gated:** the row slots are sized by the action count (`MAX_MENU_ITEMS`: each row is a different action) and `listCount()` / `buildScreen()` are capped at them, since the new row can make 17 rows (upstream had exactly 16 slots for 16) |
