...
| `lib/I18n/translations/english.yaml` | **No marker (YAML):** the `STR_LEXI_*` keys, one block after `STR_DICT_LOW_MEMORY` (the notices, then `STR_LEXI_CARD_*` for every card word, P6). (P7) `STR_LEXIRISE` and `STR_LEXI_SET_*` for the settings screen. (P9) `STR_LEXI_CARD_NEXT_SENTENCE_FAILED`, `STR_LEXI_BOOK_LANGUAGE(_AUTO)`. (M) `STR_LEXIPOINT` ("Lexipoint"); `STR_LEXI_SET_SAME_AS_CROSSPOINT` became `STR_LEXI_SET_SAME_AS_READER`, "Same as reader" (H13's default, until claritise signs it off). Other languages fall back to English. `scripts/lexipoint/test_card_strings.py` checks them against `CardStrings`; re-check when taking a CrossPoint change (gen_i18n.py rejects comments in the file) |
