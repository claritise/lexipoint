...
| D2 | **Target** | **`env:x4pro` only.** X4 Classic (`env:x4c`, S3, no touch, more buttons) is a later target. It needs its own button-only input path, since the X4 Pro design is touch-first (D15). X4/X3 (C3) are **out of scope**: their TLS budget is a separate project |
...
| D5 | **Sentence = page-bounded, base text only** | Built from the current page's `TextBlock`s. It is cut at `。！？!?` and at a paragraph end, keeps closing brackets (`」』）`), is **capped at 120 codepoints** around the tap, and **excludes ruby text**. It doesn't cross a page boundary in v0.1 (`sentence-extraction.md`) |
...
| D7 | **HTTP stack: `esp_http_client` with the CA bundle** | Same as `HttpDownloader` (`crt_bundle_attach = esp_crt_bundle_attach`). **Never `setInsecure()`**. KOSync does, but we are sending a bearer key. Responses stream into `StreamingJsonParser` (already in `lib/JsonParser`), so the whole body is never buffered (`lexirise-client.md`) |
...
| D19 | **Repo layout** (2026-09-24, claritise: docs at the root, fork in `~/Projects`) | Two sibling repos. `lexipoint` = project home with `docs/` at the root. `crosspoint-reader` = the firmware fork (`github.com/claritise/crosspoint-reader`, branch `lexipoint`), cloned at `~/Projects/crosspoint-reader`. There's no submodule. Docs refer to firmware paths relative to that repo. Build commands run there |
