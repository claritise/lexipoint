...
- The local CrossPoint fork (~/Projects/crosspoint-reader, lib/Xtc; since phase M, `firmware/lib/Xtc` in the Lexipoint repo) accepts only the XTC and XTCH magic values and contains no `xtcz`/LZ4 references. — local source inspection of `lib/Xtc/Xtc/XtcParser.cpp:132-134`, `XtcTypes.h`
