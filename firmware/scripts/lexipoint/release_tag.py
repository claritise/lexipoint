#!/usr/bin/env python3
"""The fork's release tags (firmware-base.md §6): `<upstream>-lexi.<n>`, with `-rc` for a prerelease, built
...
--releases: the fork's published (non-pre)release tags, one per line (`gh release list --limit
...
RC_SUFFIX = "-rc"
# The OTA updater keeps a tag in ReleaseJsonParser's 32-byte buffer and names the asset
# "crosspoint-<tag>-x4pro.bin" in a 48-byte one (OtaUpdater.cpp): 48 - len("crosspoint--x4pro.bin") - 1.
MAX_TAG_LENGTH = 48 - len("crosspoint--x4pro.bin") - 1
RELEASE_LIST_LIMIT = 100  # release.yml's `gh release list --limit`: plenty for a hobby fork's releases
MAX_NUMBER = 1000000  # src/lexirise/ota/ReleaseVersion.cpp kMaxVersionComponent: past it, not a version
UPSTREAM_VERSION = re.compile(r"^\d+\.\d+\.\d+$")
...
def check(tag: str, version: str, release: str, prerelease: bool, latest: str = "") -> list[str]:
    ...
    errors = []
    if not UPSTREAM_VERSION.match(version):
    ...
    if tag != want:
        errors.append(f"tag {tag!r} isn't {want!r} (no leading v: devices look for crosspoint-<tag>-x4pro.bin)")
    ...
    if latest:
        ...
        if new and old and new <= old:
            errors.append(f"tag {tag!r} isn't newer than the latest release {latest!r}: devices would never "
                          "be offered it (bump [lexirise] release; reset it to 1 only with a new upstream version)")
    return errors
...
def main(argv: list[str]) -> int:
    ...
    c.add_argument("--releases", help="the fork's published release tags, one per line, any order ('-': stdin)")
