...
```bash
curl http://lexipoint.local/api/status
```
...
```bash
curl "http://lexipoint.local/api/files?path=/Books"
```
...
```bash
curl -OJ "http://lexipoint.local/download?path=/Books/MyBook.epub"
```
...
```bash
curl -X POST -F "file=@mybook.epub" "http://lexipoint.local/upload?path=/Books"
```
...
```bash
curl -X POST -d "name=NewFolder&path=/" http://lexipoint.local/mkdir
```
...
```bash
curl -X POST -d "path=/Books/old.epub&name=new.epub" http://lexipoint.local/rename
```
...
```bash
curl -X POST -d "path=/Books/mybook.epub&dest=/Read" http://lexipoint.local/move
```
...
```bash
curl -X POST -d "path=/Books/mybook.epub" http://lexipoint.local/delete
curl -X POST -d 'paths=["/Books/old.epub","/OldFolder"]' http://lexipoint.local/delete
```
...
```bash
curl http://lexipoint.local/api/settings
```
...
curl -X POST \
  ...
  http://lexipoint.local/api/settings
```
...
```bash
curl http://lexipoint.local/api/fonts
```
...
curl -X POST \
  ...
  http://lexipoint.local/api/fonts/upload
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"family":"Literata"}' \
  http://lexipoint.local/api/fonts/delete
```
...
```bash
curl http://lexipoint.local/api/wifi
```
...
- `lexipoint.local` is advertised with mDNS when available.
- `/api/status` returns `"mode": "STA"` and RSSI in dBm.

### Access Point Mode (AP)

- Device creates an open hotspot named `Lexipoint`.
